...
class ChatResultHandler
{
    ...
    private function handleChatResult(): void
    {
        ...
        if ($this->finishReason != 'stop') {
            throw new Exception(trans('plugins/auto-content-openai::content.error.Incomplete returned content'));
        }

        if ($this->resultContent && $this->finishReason != 'stop') {
            throw new Exception(trans('plugins/auto-content-openai::content.error.An error occurred while processing the api'));
        }
    }
